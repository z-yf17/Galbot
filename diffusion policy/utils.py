    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            for cam_name in self.camera_names:
                image_dict[cam_name] = root[f'image_diagonal_view'][start_ts]
                #print(np.shape(root[f'image_diagonal_view'][start_ts]))
                #exit()
                #image_dict[cam_name] = torch.zeros((120,160,3)).float()
...
def load_data(dataset_dir_l, name_filter, camera_names, batch_size_train, batch_size_val, chunk_size, skip_mirrored_data=False, load_pretrain=False, policy_class=None, stats_dir_l=None, sample_weights=None, train_ratio=0.8):
    ...
    train_dataloader = DataLoader(train_dataset, batch_sampler=batch_sampler_train, pin_memory=True, num_workers=train_num_workers, prefetch_factor=2, collate_fn=safe_collate)
    val_dataloader = DataLoader(val_dataset, batch_sampler=batch_sampler_val, pin_memory=True, num_workers=val_num_workers, prefetch_factor=2, collate_fn=safe_collate)
