...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    #model_path = os.path.join(ckpt_dir, f'policy_step_20000_seed_0.ckpt')
    model_path = '/home/zhangyifan/Imitation/debug/act-plus-plus/ckpt/grasp_target_2/policy_step_96000_seed_0.ckpt'
    policy.deserialize(torch.load(model_path))
        ...
        if step % validate_every == 0:
            ...
            with torch.inference_mode():
                ...
                for batch_idx, data in enumerate(val_dataloader):
                    ...
                    all_actions = policy(qpos.cuda(), image.cuda())
                    print(len(data[0]))
                    print(np.shape(all_actions))
                    print(np.shape(action))

                    batch_size, horizon, dim = all_actions.shape

                    for b in range(batch_size):
                        fig, axes = plt.subplots(dim, 1, figsize=(10, 2*dim), sharex=True)
    
                        for d in range(dim):
                            axes[d].plot(range(horizon), all_actions[b, :, d].cpu().numpy(), label="all_actions", color="blue")
                            axes[d].plot(range(horizon), action[b, :, d].cpu().numpy(), label="action", color="red", linestyle="--")
                            axes[d].set_ylabel(f"dim {d}")
                            axes[d].legend(loc="upper right", fontsize=8)
    
                        axes[-1].set_xlabel("time step")
                        plt.tight_layout()
                        plt.savefig(f"fig/batch_{b}.png")  # 保存成 batch_0.png, batch_1.png ...
                        plt.close(fig)

                        

                    all_l2 = F.l1_loss(all_actions, action.cuda(), reduction='none')
                    #print(all_l2.max())
                    #print(all_l2.min())
                    loss = (all_l2 * ~is_pad.cuda().unsqueeze(-1)).mean()
                    #print(torch.sum(~is_pad))
                    print(loss)
