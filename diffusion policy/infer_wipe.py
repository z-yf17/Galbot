...
def random_target_area(xyz):
    old_pos = 'pos="0.55555 0. 0."'
    new_pos = f'pos="{xyz[0]} {xyz[1]} {xyz[2]}"'

    with open("franka_emika_panda/scene_wipe.xml", "r", encoding="utf-8") as f:
        xml_str_q = f.read()
    xml_str_q = xml_str_q.replace(old_pos, new_pos)

    new_file_path = "franka_emika_panda/modified_scene_wipe.xml"
...
def inference():
    ...
    for i in range(10000,20200,500):
        model_file = f"ckpt/wipe_target/policy_step_{i}_seed_0.ckpt"
        ...
        for test_num in range(100):
            ...
            x = 0.65 + random.random() * 0.11
            y = -0.15 + random.random() * 0.3
            target_area = [x, y, 0]
            random_target_area(target_area)
            env = Env_q()
            #env.load_model(param)
            #env.policy.load_state_dict(torch.load(model_file))

            env = Env_q()
            env.load_model()
            env.policy.deserialize(torch.load(model_file))

            env.target_area = target_area
            
            env.initial_model()
            env.initial_episode()
            env.infer(f'video/wipe/episode_{test_num + 1}.mp4')
...
def save_results(result, model_epoch, test_num):
    with open('inference_result/result-wipe_40.txt', 'a') as file:
...
class Env_q():
    def __init__(self, path_q=None):
        super().__init__()
        self.path_q = path_q if path_q else "franka_emika_panda/modified_scene_wipe.xml"
        ...
        self.result = []
        with open('ckpt/wipe_target/dataset_stats.pkl', 'rb') as file:
    ...
    def initial_model(self):
        ...
        new_state[0:7] = [0, 0.46, 0, -2, 0, 2.3, 3.1415926 / 4] + rand
        new_state[7:9] = 0
        self.physics_q.set_state(np.array(new_state))
        self.physics_q.forward()

    def initial_episode(self):
        x = 0.62 + random.random()*0.15
        y = -0.15 + random.random()*0.3

        while True:
            x = 0.62 + random.random() * 0.13
            y = -0.15 + random.random() * 0.3
            if(self.target_area[0] - x>0.02 and np.abs(self.target_area[1] - y)>0.05):
                break
    ...
    def judge_succeed(self):
        ...
        dis_x = red_box_position[0] - self.target_area[0]
        dis_y = red_box_position[1] - self.target_area[1]
        print(dis_x, dis_y, self.clean)

        #if (np.abs(dis_x) < 0.04 and np.abs(dis_y) < 0.04 and red_box_position[2] > 0.04 and self.clean):
        if (self.clean):
            self.success = True
            print("success")
            return True
    ...
    def judge_clean(self):
        ...
        if (np.abs(dis_x) < 0.02 and np.abs(dis_y) < 0.02 and red_box_position[2]<0.02):
            self.clean = True
            self.physics_q.named.model.geom_rgba['dot'] = [0.8, 0.8, 0.8, 1.0]
